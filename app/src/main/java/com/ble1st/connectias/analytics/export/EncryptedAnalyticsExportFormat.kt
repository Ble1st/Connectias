 ...
 *
 * Separate from SAF handling so it can be unit-tested.
 */
object EncryptedAnalyticsExportFormat {
    ...
    internal const val MAGIC: String = "CONNECTIAS_ANALYTICS_EXPORT"
    ...
    fun encryptZipTo(
        ...
        CipherOutputStream(dos, cipher).use { cipherOut ->
            ZipOutputStream(cipherOut).use { zipOut ->
                ...
                putTextEntry("analytics.json", json.encodeToString(exportBundle))
                putTextEntry("plugin_stats.csv", AnalyticsExportCsv.pluginStatsCsv(exportBundle.pluginStats))
            }
        }
