...
class PasswordAnalyzerTest {
    ...
    @Test
    fun `analyzePasswordStrength returns WEAK for empty password`() = runTest {
    ...
    @Test
    fun `analyzePasswordStrength returns WEAK for short simple password`() = runTest {
        val (strength, _) = analyzer.analyzePasswordStrength("12345")
        assertEquals(PasswordStrength.WEAK, strength)
    }
    
    @Test
    fun `analyzePasswordStrength returns WEAK for only letters short`() = runTest {
        val (strength, _) = analyzer.analyzePasswordStrength("password")
        assertEquals(PasswordStrength.WEAK, strength)
    }

    @Test
    fun `analyzePasswordStrength returns MEDIUM for decent length mixed case`() = runTest {
    ...
    @Test
    fun `analyzePasswordStrength returns STRONG for complex password`() = runTest {
    ...
    @Test
    fun `analyzePasswordStrength returns VERY_STRONG for long complex password`() = runTest {
