#!/bin/bash
# Build script for Rust password generator library
...
echo "Building Rust password generator for Android..."
...
for RUST_ABI in "${!ABI_MAP[@]}"; do
    ...
    if [ -f "${TARGET_DIR}/libconnectias_password_generator.so" ]; then
        cp "${TARGET_DIR}/libconnectias_password_generator.so" "${OUTPUT_LIB_DIR}/"
