...
class PluginAnalyticsStore @Inject constructor(
    ...
    private val sessionsFile: File by lazy { File(dir, "sessions.jsonl") }
    private val lifecycleFile: File by lazy { File(dir, "lifecycle_events.jsonl") }
    ...
    suspend fun appendSessionEvent(event: PluginSessionEvent) = withContext(Dispatchers.IO) {
        appendJsonLine(sessionsFile, json.encodeToString(event))
    }

    suspend fun appendLifecycleEvent(event: PluginLifecycleEvent) = withContext(Dispatchers.IO) {
        appendJsonLine(lifecycleFile, json.encodeToString(event))
    }
    ...
    suspend fun readSessionsSince(sinceEpochMillis: Long): List<PluginSessionEvent> =
        withContext(Dispatchers.IO) { readJsonLines(sessionsFile) { json.decodeFromString(it) } }

    suspend fun readLifecycleEventsSince(sinceEpochMillis: Long): List<PluginLifecycleEvent> =
        withContext(Dispatchers.IO) { readJsonLines(lifecycleFile) { json.decodeFromString(it) } }
    ...
    suspend fun compactRetention(retentionDays: Int) = withContext(Dispatchers.IO) {
        ...
        compactFile(sessionsFile, cutoff)
        compactFile(lifecycleFile, cutoff)
    }
