...
class _PluginManagerScreenState extends State<PluginManagerScreen>
          ...
          _plugins.add(
            PluginModel(
              ...
              lastUsed: DateTime.now().subtract(
                Duration(minutes: id.hashCode % 60),
              ),
  ...
  String _getPluginDescription(String id) {
    if (id.contains('storage')) {
      return 'Plugin for storage management and file operations';
    }
    if (id.contains('network')) {
      return 'Plugin for network communication and API calls';
    }
    if (id.contains('security')) {
      return 'Plugin for security monitoring and protection';
    }
    if (id.contains('utility')) {
      return 'Utility plugin for various system functions';
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: _loadPlugins),
    ...
    if (updatedPlugin != null && mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Plugin ${updatedPlugin.name} wurde ${updatedPlugin.status == PluginStatus.active ? 'aktiviert' : 'deaktiviert'}',
            ),
...
enum PluginSortOption { name, status, memory, lastUsed }

/// Plugin Installation Dialog
class PluginInstallDialog extends StatefulWidget {
  final Function(File) onInstall;

  const PluginInstallDialog({super.key, required this.onInstall});
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        if (file.path != null && file.path!.isNotEmpty) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Ungültiger oder nicht unterstützter Dateipfad'),
              ),
            );
          }
        }
