...
sealed class PinValidationResult {
     ...
     */
    data class Valid(val matchedPin: String) : PinValidationResult()

    /**
     * The certificate chain does not match any pins.
     */
    data class Invalid(val reason: String, val certificateChain: List<String>) : PinValidationResult()
     ...
     */
    data class PinExpired(val hostname: String) : PinValidationResult()

    /**
     * An error occurred during validation.
     */
    data class Error(val exception: Throwable) : PinValidationResult()

    fun isValid(): Boolean = this is Valid || this is NoPinsConfigured
}
