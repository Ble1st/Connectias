...
class LoggingService @Inject constructor(
     ...
     * @return Flow of recent security logs
     */
    fun getRecentLogs(limit: Int = 100): Flow<List<SecurityLogEntity>> {
        return securityLogDao.getRecentLogs(limit)
    }
    
    /**
     * Gets logs filtered by threat type.
     * 
     * @param threatType The threat type to filter by
     * @param limit Maximum number of logs to retrieve (default: 100)
     * @return Flow of filtered security logs
     */
    fun getLogsByThreatType(threatType: String, limit: Int = 100): Flow<List<SecurityLogEntity>> {
        return securityLogDao.getLogsByThreatType(threatType, limit)
    }
    ...
    /**
     * Gets logs filtered by threat level.
     * 
     * @param threatLevel The threat level to filter by (CRITICAL, HIGH, MEDIUM, WARNING, INFO)
     * @param limit Maximum number of logs to retrieve (default: 100)
     * @return Flow of filtered security logs
     */
    fun getLogsByThreatLevel(threatLevel: String, limit: Int = 100): Flow<List<SecurityLogEntity>> {
        return securityLogDao.getLogsByThreatLevel(threatLevel, limit)
    }
}
