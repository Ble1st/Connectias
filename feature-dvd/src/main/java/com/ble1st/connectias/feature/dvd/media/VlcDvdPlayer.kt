...
class VlcDvdPlayer(private val context: Context) {
            ...
            if (devicePath.startsWith("/") && !devicePath.startsWith("content://")) {
                ...
                try {
                    ...
                    media = withContext(Dispatchers.IO) {
                        ...
                        val uri = Uri.parse("dvd://$devicePath")
    ...
    private fun createMediaFromHandle(libVlc: LibVLC, nativeHandle: Long): Media? {
        Timber.d("VlcDvdPlayer: createMediaFromHandle() called - nativeHandle: $nativeHandle")
        try {
            Timber.d("VlcDvdPlayer: createMediaFromHandle() - Getting Media class constructors")
            ...
            Timber.d("VlcDvdPlayer: createMediaFromHandle() - Found ${constructors.size} constructors")
            for ((index, constructor) in constructors.withIndex()) {
                val params = constructor.parameterTypes
                Timber.d("VlcDvdPlayer: createMediaFromHandle() - Constructor $index: ${params.size} parameters")
                if (params.size == 2) {
                    Timber.d("VlcDvdPlayer: createMediaFromHandle() - Constructor $index: param[0]=${params[0].name}, param[1]=${params[1].name}")
                    if (org.videolan.libvlc.interfaces.ILibVLC::class.java.isAssignableFrom(params[0]) &&
                        (params[1] == Long::class.javaPrimitiveType || params[1] == Long::class.java)) {
                        Timber.d("VlcDvdPlayer: createMediaFromHandle() - Constructor $index matches signature, creating instance")
                        constructor.isAccessible = true
                        val media = constructor.newInstance(libVlc, nativeHandle) as Media
                        Timber.d("VlcDvdPlayer: createMediaFromHandle() - Media instance created successfully: $media")
                        return media
                    }
                }
            }
            Timber.e("VlcDvdPlayer: createMediaFromHandle() - No matching constructor found")
        } catch (e: Exception) {
            Timber.e(e, "VlcDvdPlayer: createMediaFromHandle() - Failed to instantiate Media via reflection")
        }
        Timber.e("VlcDvdPlayer: createMediaFromHandle() - Returning null")
        return null
    }
    ...
    fun navigateUp() {
        Timber.d("VlcDvdPlayer: navigateUp() called")
        val result = mediaPlayer?.navigate(MediaPlayer.Navigate.Up)
        Timber.d("VlcDvdPlayer: navigateUp() - Result: $result")
    }
    fun navigateDown() {
        Timber.d("VlcDvdPlayer: navigateDown() called")
        val result = mediaPlayer?.navigate(MediaPlayer.Navigate.Down)
        Timber.d("VlcDvdPlayer: navigateDown() - Result: $result")
    }
    fun navigateLeft() {
        Timber.d("VlcDvdPlayer: navigateLeft() called")
        val result = mediaPlayer?.navigate(MediaPlayer.Navigate.Left)
        Timber.d("VlcDvdPlayer: navigateLeft() - Result: $result")
    }
    fun navigateRight() {
        Timber.d("VlcDvdPlayer: navigateRight() called")
        val result = mediaPlayer?.navigate(MediaPlayer.Navigate.Right)
        Timber.d("VlcDvdPlayer: navigateRight() - Result: $result")
    }
    fun navigateEnter() {
        Timber.d("VlcDvdPlayer: navigateEnter() called")
        val result = mediaPlayer?.navigate(MediaPlayer.Navigate.Activate)
        Timber.d("VlcDvdPlayer: navigateEnter() - Result: $result")
    }
    ...
    fun ioRead(buffer: ByteArray, size: Int): Int {
        ...
        try {
            ...
            val bytesToReadFromDevice = (blocksToRead * blockSize).toInt()
    ...
    fun ioSeek(offset: Long): Boolean {
        ...
        if (vobOffsets != null && totalVobSize > 0) {
            if (offset < 0 || offset > totalVobSize) {
    ...
    fun attachView(surfaceView: SurfaceView) {
        ...
        if (vout != null) {
            ...
            mediaPlayer?.setScale(0f) // auto-scale to fit
            mediaPlayer?.setAspectRatio(null) // reset to default
    ...
    fun detachView() {
        Timber.d("VlcDvdPlayer: detachView() called")
        val vout = mediaPlayer?.vlcVout
        Timber.d("VlcDvdPlayer: detachView() - vout: $vout")
        if (vout != null) {
            Timber.d("VlcDvdPlayer: detachView() - Detaching views")
            vout.detachViews()
            Timber.d("VlcDvdPlayer: detachView() - Views detached")
        } else {
            Timber.w("VlcDvdPlayer: detachView() - vout is null")
        }
    }
