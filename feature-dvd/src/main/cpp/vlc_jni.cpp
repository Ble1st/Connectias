...
typedef jlong (*getLibVLCInstance_t)(JNIEnv *env, jobject libvlcObj);
static void *g_libvlcjni_handle = nullptr;
static getLibVLCInstance_t g_getLibVLCInstance = nullptr;
...
Java_com_ble1st_connectias_feature_dvd_media_VlcDvdPlayer_nativeReleaseMedia(JNIEnv *env, jobject clazz, jlong mediaHandle) {
    LOGD("VlcJni: nativeReleaseMedia() called - mediaHandle: %ld", (long)mediaHandle);
    if (mediaHandle != 0) {
        ...
        if (g_libvlc_media_release) {
            ...
            LOGE("VlcJni: nativeReleaseMedia() - g_libvlc_media_release is null, cannot release");
        }
    } else {
        LOGD("VlcJni: nativeReleaseMedia() - Media handle is 0, nothing to release");
    }
    LOGD("VlcJni: nativeReleaseMedia() - Complete");
}
