...
/** CSS IOCTL operation types for stream callback */
#define DVDCSS_IOCTL_REPORT_AGID       0x01
#define DVDCSS_IOCTL_REPORT_CHALLENGE  0x02
#define DVDCSS_IOCTL_REPORT_KEY1       0x03
#define DVDCSS_IOCTL_REPORT_TITLE_KEY  0x04
#define DVDCSS_IOCTL_REPORT_ASF        0x05
#define DVDCSS_IOCTL_REPORT_RPC        0x06
#define DVDCSS_IOCTL_SEND_CHALLENGE    0x11
#define DVDCSS_IOCTL_SEND_KEY2         0x12
#define DVDCSS_IOCTL_INVALIDATE_AGID   0x3F
#define DVDCSS_IOCTL_READ_COPYRIGHT    0x20
#define DVDCSS_IOCTL_READ_DISC_KEY     0x21
...
typedef struct dvdcss_stream_cb
{
    ...
    /** 
     * Custom ioctl callback for CSS operations.
     * @param p_stream User-provided stream context
     * @param i_op Operation type (DVDCSS_IOCTL_*)
     * @param p_data Input/output data buffer
     * @param i_data_size Size of data buffer
     * @param p_agid Pointer to AGID (may be input, output, or both depending on operation)
     * @param i_lba LBA for title key operations (0 for other operations)
     * @return 0 on success, negative on error
     */
    int ( *pf_ioctl ) ( void *p_stream, int i_op, void *p_data, int i_data_size, 
                        int *p_agid, int i_lba);
} dvdcss_stream_cb;
