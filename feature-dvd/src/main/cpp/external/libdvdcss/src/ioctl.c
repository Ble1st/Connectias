 ...
 *****************************************************************************/
int ioctl_ReadCPRMMediaId(int i_fd,const int *p_agid, uint8_t *p_buffer)
{  
    int i_ret;
 ...
 *****************************************************************************/
int ioctl_ReadCPRMMKBPack(int i_fd, const int *p_agid, int mkb_pack, uint8_t *p_mkb_pack, int *p_total_packs)
{
    int i_ret;
...
/*****************************************************************************
 * dvdcss-aware IOCTL wrapper functions
 *****************************************************************************
 * These functions check if a stream callback with pf_ioctl is available
 * and use it for CSS operations, otherwise fall back to direct ioctl calls.
 *****************************************************************************/
#include "libdvdcss.h"

/* Helper to check if we should use the stream callback */
static inline int use_stream_ioctl( dvdcss_t dvdcss )
{
    return dvdcss->p_stream && dvdcss->p_stream_cb && 
           dvdcss->p_stream_cb->pf_ioctl;
}

int css_ReadCopyright( dvdcss_t dvdcss, int i_layer, int *pi_copyright )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = 0;
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream, 
            DVDCSS_IOCTL_READ_COPYRIGHT, pi_copyright, sizeof(int), &agid, i_layer );
    }
    return ioctl_ReadCopyright( dvdcss->i_fd, i_layer, pi_copyright );
}

int css_ReadDiscKey( dvdcss_t dvdcss, const int *pi_agid, uint8_t *p_key )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = *pi_agid;
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_READ_DISC_KEY, p_key, DVD_DISCKEY_SIZE, &agid, 0 );
    }
    return ioctl_ReadDiscKey( dvdcss->i_fd, pi_agid, p_key );
}

int css_ReadTitleKey( dvdcss_t dvdcss, const int *pi_agid, int i_pos, uint8_t *p_key )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = *pi_agid;
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_REPORT_TITLE_KEY, p_key, DVD_KEY_SIZE, &agid, i_pos );
    }
    return ioctl_ReadTitleKey( dvdcss->i_fd, pi_agid, i_pos, p_key );
}

int css_ReportAgid( dvdcss_t dvdcss, int *pi_agid )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_REPORT_AGID, NULL, 0, pi_agid, 0 );
    }
    return ioctl_ReportAgid( dvdcss->i_fd, pi_agid );
}

int css_ReportChallenge( dvdcss_t dvdcss, const int *pi_agid, uint8_t *p_challenge )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = *pi_agid;
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_REPORT_CHALLENGE, p_challenge, DVD_CHALLENGE_SIZE, &agid, 0 );
    }
    return ioctl_ReportChallenge( dvdcss->i_fd, pi_agid, p_challenge );
}

int css_ReportKey1( dvdcss_t dvdcss, const int *pi_agid, uint8_t *p_key )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = *pi_agid;
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_REPORT_KEY1, p_key, DVD_KEY_SIZE, &agid, 0 );
    }
    return ioctl_ReportKey1( dvdcss->i_fd, pi_agid, p_key );
}

int css_ReportASF( dvdcss_t dvdcss, int *pi_asf )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = 0;
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_REPORT_ASF, pi_asf, sizeof(int), &agid, 0 );
    }
    return ioctl_ReportASF( dvdcss->i_fd, pi_asf );
}

int css_InvalidateAgid( dvdcss_t dvdcss, int *pi_agid )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_INVALIDATE_AGID, NULL, 0, pi_agid, 0 );
    }
    return ioctl_InvalidateAgid( dvdcss->i_fd, pi_agid );
}

int css_SendChallenge( dvdcss_t dvdcss, const int *pi_agid, const uint8_t *p_challenge )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = *pi_agid;
        /* Cast away const for the callback - data is not modified for SEND operations */
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_SEND_CHALLENGE, (void*)p_challenge, DVD_CHALLENGE_SIZE, &agid, 0 );
    }
    return ioctl_SendChallenge( dvdcss->i_fd, pi_agid, p_challenge );
}

int css_SendKey2( dvdcss_t dvdcss, const int *pi_agid, const uint8_t *p_key )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        int agid = *pi_agid;
        /* Cast away const for the callback - data is not modified for SEND operations */
        return dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_SEND_KEY2, (void*)p_key, DVD_KEY_SIZE, &agid, 0 );
    }
    return ioctl_SendKey2( dvdcss->i_fd, pi_agid, p_key );
}

int css_ReportRPC( dvdcss_t dvdcss, int *p_type, int *p_mask, int *p_scheme )
{
    if( use_stream_ioctl( dvdcss ) )
    {
        /* Pack RPC info into a buffer: [type, mask, scheme] */
        uint8_t rpc_buffer[3] = { 0 };
        int agid = 0;
        int ret = dvdcss->p_stream_cb->pf_ioctl( dvdcss->p_stream,
            DVDCSS_IOCTL_REPORT_RPC, rpc_buffer, 3, &agid, 0 );
        if( ret == 0 )
        {
            *p_type = rpc_buffer[0];
            *p_mask = rpc_buffer[1];
            *p_scheme = rpc_buffer[2];
        }
        return ret;
    }
    return ioctl_ReportRPC( dvdcss->i_fd, p_type, p_mask, p_scheme );
}
