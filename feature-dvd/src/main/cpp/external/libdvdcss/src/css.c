...
int dvdcss_test( dvdcss_t dvdcss )
{
    ...
    i_ret = css_ReadCopyright( dvdcss, 0 /* i_layer */, &i_copyright );
    ...
    i_ret = css_ReportRPC( dvdcss, &i_type, &i_mask, &i_rpc);
...
int dvdcss_disckey( dvdcss_t dvdcss )
{
    ...
    if( css_ReadDiscKey( dvdcss, &dvdcss->css.i_agid, p_buffer ) < 0 )
    {
    ...
    if( GetASF( dvdcss ) != 1 )
    {
        ...
        css_InvalidateAgid( dvdcss, &dvdcss->css.i_agid );
        return -1;
    }
...
static int dvdcss_titlekey( dvdcss_t dvdcss, int i_pos, dvd_key p_title_key )
{
                              ...
                              dvdcss->i_method == DVDCSS_METHOD_DISC ) )
    {
        ...
        if( css_ReadTitleKey( dvdcss, &dvdcss->css.i_agid,
                                i_pos, p_key ) < 0 )
        {
            print_debug( dvdcss,
...
int GetBusKey( dvdcss_t dvdcss )
{
    ...
    i_ret = css_ReportAgid( dvdcss, &dvdcss->css.i_agid );
    ...
    /* Get key1 from LU */
    if( css_ReportKey1( dvdcss, &dvdcss->css.i_agid, p_buffer ) < 0)
    {
    ...
    /* Send key2 to LU */
    if( css_SendKey2( dvdcss, &dvdcss->css.i_agid, p_buffer ) < 0 )
    {
...
int GetASF( dvdcss_t dvdcss )
{
    ...
    if( css_ReportASF( dvdcss, &i_asf ) != 0 )
    {
