...
{
    /* For stream-based access, check if ioctl callback is provided */
    if( dvdcss->p_stream )
    {
        /* If the stream has an ioctl callback, we can use CSS operations */
        if( dvdcss->p_stream_cb && dvdcss->p_stream_cb->pf_ioctl )
            return 1;
        return 0;
    }
